...
final class SpotlightIndexingTests: XCTestCase {
    ...
    func testIdentifierIsRootScopedAbsolutePath() {
        // The Spotlight uid is the ABSOLUTE path (unique across libraries), not
        // the bare relative path — the relative path is the display subtitle.
        let id = SpotlightIndexing.identifier(
            forDocumentPath: "/Lib/Notes/Today.md", relativeTo: "/Lib")
        XCTAssertEqual(id, "/Lib/Notes/Today.md")
        XCTAssertEqual(
            SpotlightIndexing.relativePath(
                forDocumentPath: "/Lib/Notes/Today.md", relativeTo: "/Lib"),
            "Notes/Today.md")
    }

    func testIdentifierIsUniqueAcrossLibrariesSharingARelativePath() {
        // Two roots each containing `Notes/Today.md` MUST produce distinct
        // identifiers, or the app-wide index would collide (overwrite,
        // cross-delete, wrong-file open). This is the core of finding #1.
        let a = SpotlightIndexing.identifier(
            forDocumentPath: "/Work/Notes/Today.md", relativeTo: "/Work")
        let b = SpotlightIndexing.identifier(
            forDocumentPath: "/Personal/Notes/Today.md", relativeTo: "/Personal")
        XCTAssertEqual(a, "/Work/Notes/Today.md")
        XCTAssertEqual(b, "/Personal/Notes/Today.md")
        XCTAssertNotEqual(a, b)
        // Yet both share the same human-facing relative path / subtitle.
        XCTAssertEqual(
            SpotlightIndexing.relativePath(forDocumentPath: "/Work/Notes/Today.md", relativeTo: "/Work"),
            SpotlightIndexing.relativePath(forDocumentPath: "/Personal/Notes/Today.md", relativeTo: "/Personal"))
    }
    ...
    func testIdentifierRoundTripsBackToAbsolutePath() {
        ...
        XCTAssertEqual(id, path)
        XCTAssertEqual(
            SpotlightIndexing.relativePath(forDocumentPath: path, relativeTo: root), "Deep/Sub/Note.md")
        // The absolute id resolves back to itself under its own root, and only
        // under its own root — a different root refuses it (routing safety).
        XCTAssertEqual(
            SpotlightIndexing.documentPath(forIdentifier: id, relativeTo: root), path)
        XCTAssertNil(
            SpotlightIndexing.documentPath(forIdentifier: id, relativeTo: "/Users/me/Other"))
    }
    ...
    func testIdentifierRoundTripSurvivesSpacesAndUnicode() {
        ...
        XCTAssertEqual(id, path)
        XCTAssertEqual(
            SpotlightIndexing.relativePath(forDocumentPath: path, relativeTo: root), "My Notes/Café Ideas.md")
        XCTAssertEqual(
            SpotlightIndexing.documentPath(forIdentifier: id, relativeTo: root), path)
    }

    func testIdentifierNormalizesTraversalInInput() {
        // A `.`/`..`-laden absolute input collapses to the same canonical id as
        // its clean form, so the uid is stable regardless of how the scan
        // spelled the path.
        XCTAssertEqual(
            SpotlightIndexing.identifier(forDocumentPath: "/Lib/a/../Notes/Today.md", relativeTo: "/Lib"),
            "/Lib/Notes/Today.md")
    }
    ...
    func testIdentifierRefusesDocumentOutsideRoot() {
        ...
        XCTAssertNil(SpotlightIndexing.relativePath(
            forDocumentPath: "/Elsewhere/a.md", relativeTo: "/Lib"))
    }
    ...
    func testIndexedDocumentSnapshot() {
        ...
        let derived = SpotlightIndexing.indexedDocument(
            for: doc, identifier: "/Lib/Folder/Snapshot.md", relativePath: "Folder/Snapshot.md",
            filenameStem: "Snapshot", snippetLimit: 300)
        XCTAssertEqual(derived.identifier, "/Lib/Folder/Snapshot.md")
    ...
    func testStaleIdentifiersDetectAMove() {
        // A move looks like: old absolute path gone, new one present.
        let stale = SpotlightIndexing.staleIdentifiers(
            previouslyIndexed: ["/Lib/Old/Note.md"],
            current: ["/Lib/New/Note.md"])
        XCTAssertEqual(stale, ["/Lib/Old/Note.md"])
    }

    // MARK: - Persisted-root registry (bounded storage)

    func testRegistryTouchAppendsNewRootMostRecent() {
        let (registry, evicted) = SpotlightIndexing.prunedRootRegistry(
            ["/A", "/B"], touching: "/C", limit: 32)
        XCTAssertEqual(registry, ["/A", "/B", "/C"])
        XCTAssertTrue(evicted.isEmpty)
    }

    func testRegistryTouchMovesExistingRootToMostRecentWithoutDuplicating() {
        let (registry, evicted) = SpotlightIndexing.prunedRootRegistry(
            ["/A", "/B", "/C"], touching: "/A", limit: 32)
        XCTAssertEqual(registry, ["/B", "/C", "/A"])
        XCTAssertTrue(evicted.isEmpty)
    }

    func testRegistryEvictsLeastRecentlyUsedOverLimit() {
        // At the limit, touching a new root evicts the oldest.
        let (registry, evicted) = SpotlightIndexing.prunedRootRegistry(
            ["/A", "/B", "/C"], touching: "/D", limit: 3)
        XCTAssertEqual(registry, ["/B", "/C", "/D"])
        XCTAssertEqual(evicted, ["/A"])
    }

    func testRegistryEvictsMultipleWhenFarOverLimit() {
        let (registry, evicted) = SpotlightIndexing.prunedRootRegistry(
            ["/A", "/B", "/C", "/D", "/E"], touching: "/F", limit: 2)
        XCTAssertEqual(registry, ["/E", "/F"])
        XCTAssertEqual(evicted, ["/A", "/B", "/C", "/D"])
    }

    func testRegistryTouchingExistingRootAtLimitEvictsNothing() {
        // Re-touching an already-present root never grows the set, so nothing
        // is evicted even at the limit.
        let (registry, evicted) = SpotlightIndexing.prunedRootRegistry(
            ["/A", "/B", "/C"], touching: "/B", limit: 3)
        XCTAssertEqual(registry, ["/A", "/C", "/B"])
        XCTAssertTrue(evicted.isEmpty)
    }
}
