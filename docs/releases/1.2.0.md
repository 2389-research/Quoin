...
- **Single-file editing has no library clutter.** Open one `.md` from Finder and the window is a full-width editor — no empty library pane.
- **Your tabs come back after quitting.** Open tabs, active document, and layout now survive quit → relaunch even with macOS's "Close windows when quitting an application" turned on.
